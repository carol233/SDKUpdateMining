...
public class ArgumentValueManager {
  ...
  private final Map<ArgumentValueIdentifier, List<Object>> cachedValues = new HashMap<>();
   ...
   */
  public List<Object> getArgumentValues(Argument argument, Unit callSite) {
    ...
    if (AnalysisParameters.v().isIterative()) {
      ...
      List<Object> result = cachedValues.get(argumentValueIdentifier);
      if (result != null) {
        return result;
      } else {
   ...
   * @return The possible argument values.
   */
  private List<Object> computeNewArgumentValues(Argument argument, Unit callSite) {
    String type = argument.getType();
    ...
    if (inlineValues == null) {
      ...
      Future<List<Object>> valuesFuture =
      ...
      try {
        ...
        return Collections.singletonList(analysis.getTopValue());
      } catch (Error error) {
   ...
   */
  private Future<List<Object>> startComputingNewArgumentValues(final ExecutorService pool,
