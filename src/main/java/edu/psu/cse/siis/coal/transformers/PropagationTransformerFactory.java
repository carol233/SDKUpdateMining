...
public class PropagationTransformerFactory {
   ...
   * @param argument An {@link Argument Argument}.
  ...
  private static PropagationTransformer
    ...
    if (Model.v().isModeledType(argument.getType())) {
      ...
      List<Object> values = ArgumentValueManager.v().getArgumentValues(argument, callSite);

      makePlainTransformer(new HashSet<>(values), transformer, argument, callSite);
    }
   ...
   * @param argument The COAL modifier {@link Argument Argument}.
   * @param callSite A call site that is a COAL modifier.
   */
   ...
   * @param argument A COAL modifier {@link Argument Argument}.
